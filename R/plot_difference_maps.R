...
plot_difference_maps  <- function( data,
  ...
  if(length(topoplots_time_windows) <13){
    ...
    topoplot_with_legend <- ggpubr::ggarrange( topo_legend, topoplot, heights = c(0.9, 4),
                                               #labels = c("ERPs", "Voltage maps"),
                                               ncol = 1, nrow =2)
  }else{
    topoplot <- ggpubr::ggarrange(plotlist=topo_ggplots, nrow = 2, ncol = ((length(topoplots_time_windows)+1)%/%2))
    topoplot_with_legend <- ggpubr::ggarrange( topo_legend, topoplot, heights = c(1.5, 12),
                                               #labels = c("ERPs", "Voltage maps"),
                                               ncol = 1, nrow =2)
  }
  ...
  if(length(topoplots_time_windows) <13){
    ...
    ggplot2::ggsave(plot= topoplot_with_legend ,filename= plot_filename, width = 2*length(topoplots_time_windows), height = 12)
  }
  t_end <- Sys.time()
