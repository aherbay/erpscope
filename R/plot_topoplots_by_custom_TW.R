    ...
    if (data_to_display == "t_test_p_value") {
          ...
          topo_ggplots[[length(topo_ggplots) + 1]] <-   ggplot2::ggplot(interpTopo,
            ...
            # version 1 with more gradient approach
             # scale_fill_steps2(midpoint = t_test_threshold, limits = c(0,0.10), oob=scales::squish, mid = scales::muted("red"), high = "white", low = scales::muted("green", guide = 'none') +
            # version 2 with more stats threshold approach

            #scale_fill_steps2(midpoint = t_test_threshold, limits = c(0,0.10), oob=scales::squish,mid = "#99cc33", high = "#339900", low = ," #ffcc00",  breaks= c(0.01,0.05))+ #, breaks= c(0.01,0.05,0.05)guide = 'none',
            ...
            #scale_fill_stepsn(  oob=scales::squish, breaks= c(0,0.001,0.01,0.05,0.05,0.10,1), colours =  c("#9EBCDA", "#8C96C6", "#8C6BB1", "#88419D", "#810F7C", "#4D004B"),guide = 'none')+ #guide = 'none',, limits = c(0,1)
            #scale_fill_stepsn(colors = c("#D73027", "#1A9850") , breaks= c(t_test_threshold)   )+ # pb it changes from on VM to another
            #scale_fill_steps2(midpoint = t_test_threshold, limits = c(0,0.10), breaks = c(0.001, 0.01, 0.05), mid = scales::muted("red"), high = "white", low = "red" , guide = 'none')+ #, breaks= c(0,0.001,0.01,0.05,0.05,0.10,1)
     ...
     if(tw_index == length(tw_array) ){
       ...
       if(data_to_display == "t_test_p_value" ) {
         ...
         topo_forlegend <-   ggplot2::ggplot(interpTopo,
           ...
           #scale_fill_stepsn( breaks= c(0,0.001,0.01,0.05,0.05,0.10,1), colours =  c("#9EBCDA", "#8C96C6", "#8C6BB1", "#88419D", "#810F7C", "#4D004B"),guide = 'none')+ #guide = 'none',, limits = c(0,1)
           #scale_fill_steps2(midpoint = t_test_threshold, limits = c(0,0.10),oob=scales::squish, mid = scales::muted("red"), high = "white", low = scales::muted("green") , guide = 'none')+ #, breaks= c(0,0.001,0.01,0.05,0.05,0.10,1)
